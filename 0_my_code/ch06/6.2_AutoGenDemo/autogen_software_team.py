# 6.2 框架一：AutoGen 
"""
首先，要安装 AutoGen （当前版本是 0.7.5 ， 跟课程中说的 0.7.4 相差不远）
pip install -U "autogen-agentchat" "autogen-ext[openai]"
"""

"""
使用 AutoGen 来构建一个模拟的软件开发团队，该团队由多个具有不同专业技能的智能体组成，它们将协作完成一个真实的软件开发任务。  
我们的目标是开发一个功能明确的 Web 应用：实时显示比特币当前价格。这个任务虽小，却完整地覆盖了软件开发的典型环节：从需求分析、技术选型、编码实现到代码审查和最终测试。  
这使其成为检验 AutoGen 自动化协作流程的理想场景。  
...
"""
这里的例子使用 OpenAI 模型配置起来方便一些。没有 OpenAI apiKey，使用 https://github.com/popjane/free_chatgpt_api 代替
...
# （2）智能体角色的定义
# 创建产品经理智能体 (注意：这是一个 AssistantAgent)
...
# 创建软件工程师智能体 (也是一个 AssistantAgent)
...
# 代码审查员 (CodeReviewer)  (也是一个 AssistantAgent)
...
# 用户代理 (UserProxy) (注意：它是一个 UserProxyAgent)
...
# 手动在 full_output.md 中保存了运行输出

# 可以看到智能体通过多轮对话，最终实现了任务
