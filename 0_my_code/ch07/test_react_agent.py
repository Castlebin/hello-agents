...
def test_react_agent():
    ...
    try:
        ...
        # 注意，这里注册的是 function，不是 tool (搞不明白为啥分了 function 和 tool，先不管)
    ...
    # 显示工具使用统计
    # 上面注册用的是 register_function，所以这里 tools 数量是 0 
    ...
    # 增加打印 functions 
    print(f"\n 注册的 function 数量 {len(tool_registry._functions)}")
    print("已注册的 functions: ")
    for function_name in tool_registry._functions:
