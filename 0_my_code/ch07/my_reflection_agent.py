...
class MyReflectionAgent(ReflectionAgent):
    ...
    def __init__(self,
        ...
        # 初始化执行、反思、改进 三阶段的提示词模板
        self.initial_prompt_template = self.prompts["initial"] or DEFAULT_PROMPTS["initial"]
        self.reflect_prompt_template = self.prompts["reflect"] or DEFAULT_PROMPTS["reflect"]
        self.refine_prompt_template = self.prompts["refine"] or DEFAULT_PROMPTS["refine"]
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        # 1. 初始执行
        print("\n--- 正在进行初始尝试 ---")
        initial_prompt = self.initial_prompt_template.format(task=input_text)
        initial_result = self._get_llm_response(initial_prompt, **kwargs)
        self.memory.add_record("execution", initial_result)
        
        reach_final_answer = False
        # 2. 迭代循环：反思与优化
        ...
        if not reach_final_answer:
            print(f"\n--- 达到迭代轮数上限 {self.max_iterations} 未能得到最终结果，任务结束 ")
            return ""
